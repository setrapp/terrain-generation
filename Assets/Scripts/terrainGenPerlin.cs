...
public class terrainGenPerlin : MonoBehaviour {
	...
	void Start () {
        ...
        for (int i = 0; i < CHUNKS; i++)
        {
            for (int j = 0; j < CHUNKS; j++)
            {
                ...
                heightMap[i * CHUNKS + j] = k_perlin(heightMap[i * CHUNKS + j], width, length);
                y++;
        ...
        for (int row = 0; row < CHUNKS; row++)
        {
            for (int col = 0; col < CHUNKS; col++)
            {
                ...
                terrain[row, col].transform.position = whereIsTerrain[row, col];
    ...
    private float[,] k_perlin(float[,] pos, int width, int height)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            {
                pos[i, j] = height2d((i) * scale + xOffset, (j) * scale + yOffset, perlinOctaves, perlinLacunarity, perlinGain);
            }
        }
